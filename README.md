...
├── routes/
│   └── api.js         ← REST API endpoints with rate limiting
└── public/
...
**Full test flow:**
```bash
# 1. Init user (creates account, returns energy + config)
curl -X POST http://localhost:3000/api/init \
  -H "Content-Type: application/json" \
  -d '{"devUserId":"agent_1"}'

# 2. Tap 10 times (server validates energy, caps to available)
...
# 4. Buy an idle upgrade
curl -X POST http://localhost:3000/api/upgrade \
  -H "Content-Type: application/json" \
  -d '{"devUserId":"agent_1","type":"idle"}'
...
# 6. Get leaderboard (no auth required)
curl http://localhost:3000/api/leaderboard

# 7. Get friends/referrals (requires devUserId)
curl "http://localhost:3000/api/friends" \
  -H "Content-Type: application/json" \
  --data-urlencode "devUserId=agent_1"

# 8. Test referral — create user2 with user1's ref code
# First get user1's referral_code from the /init response, then:
curl -X POST http://localhost:3000/api/init \
  -H "Content-Type: application/json" \
  -d '{"devUserId":"agent_2","ref":"USER1REFCODE"}'
```

**What agents should verify:**
- `user.energy` decreases with each tap and never goes below 0
- Sending `taps: 999` only earns coins for available energy
- Refreshing (re-calling `/init`) does NOT reset energy to 100
- Energy regenerates correctly over time (1 per 3 seconds)
- Upgrade balance check: buying with insufficient coins returns `{ error: "Need X 💎..." }`
- Daily reward can only be claimed once per UTC day
